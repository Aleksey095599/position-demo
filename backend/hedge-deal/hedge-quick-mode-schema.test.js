...
function quickModeDatabase() {
  ...
  database.exec(`
    ...
    VALUES
      (1, 'EXTERNAL', 'Hedge Counterparty', 1),
      (2, 'EXTERNAL', 'Client Counterparty', 1);
...
test("Hedge Quick Mode Settings enforce the preset and pricing-rule invariants", () => {
  ...
  try {
    ...
    assert.throws(
      () => database.exec(`
        UPDATE hedge_quick_mode_settings
  ...
  try {
    ...
    assert.throws(
      () => database.exec(`
        ...
        WHERE originating_system_id = 'AUTO'
      `),
  ...
  try {
    ...
    const settings = database.prepare(`
      SELECT
        ...
        originating.pricing_mode AS pricing_mode
