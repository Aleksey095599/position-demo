...
const TEMPORARY_DIRECTORY_PREFIX = "position-originating-system-pricing-mode-";
...
function legacyTradeContextDatabase({ duplicateContext = false } = {}) {
  const database = new DatabaseSync(":memory:");
  database.exec("PRAGMA foreign_keys = ON");
  database.exec(`
    ...
    INSERT INTO originating_systems VALUES
      ('VERIFY_MODE_LOCK', 'Verification Originating System', 'DEALER_APPROVED', 1);
    INSERT INTO ccy_pair_options VALUES ('EUR_USD');
    INSERT INTO trading_counterparties VALUES (1);
    INSERT INTO trade_contexts VALUES
      ('LEGACY_CONTEXT', '000', 'AFINA', 'VERIFY_MODE_LOCK');
    INSERT INTO trading_counterparty_trade_contexts VALUES (1, 'LEGACY_CONTEXT');
...
function originatingSystemPayload({
...
test("database locks Pricing Mode only while Trade Contexts are attached", () => {
  ...
  try {
    database.prepare(`
      ...
      VALUES ('VERIFY_MODE_LOCK', 'Verification Originating System', 'DEALER_APPROVED', 1)
    `).run();
    database.prepare(`
      UPDATE originating_systems
      SET pricing_mode = 'DEALER_PRICED'
      WHERE originating_system_id = 'VERIFY_MODE_LOCK'
    `).run();
    assert.equal(
      database.prepare(`
    ...
    const tradeContextId = Number(database.prepare(`
    ...
    assert.throws(() => database.prepare(`
      ...
      WHERE originating_system_id = 'VERIFY_MODE_LOCK'
    `).run(), /used by Trade Context cannot change Pricing Mode/);
    assert.deepEqual(
      { ...database.prepare(`
...
test("legacy Trade Context migration restores the Pricing Mode lock atomically", () => {
  const migrateLegacyTradeContextIds = new Function(
    `${serverTopLevelFunctionSource("migrateLegacyTradeContextIds")}; return migrateLegacyTradeContextIds;`
  )();
  const migratedDatabase = legacyTradeContextDatabase();
  const rejectedMigrationDatabase = legacyTradeContextDatabase({
    duplicateContext: true
  });

  try {
    migrateLegacyTradeContextIds(migratedDatabase);
    assert.equal(
      migratedDatabase.prepare(`
        SELECT type
        FROM pragma_table_info('trade_contexts')
        WHERE name = 'trade_context_id'
      `).get().type,
      "INTEGER"
    );
    ...
    assert.ok(migratedDatabase.prepare(`
      ...
      FROM sqlite_master
      WHERE type = 'trigger'
        AND name = 'trg_originating_systems_lock_pricing_mode_while_referenced'
    `).get());
    ...
    assert.throws(
      () => migrateLegacyTradeContextIds(rejectedMigrationDatabase),
...
test("Originating System API rejects a Pricing Mode change only while referenced", async t => {
  ...
  const changedWhileUnused = await request(
    "PUT",
    "/api/v1/originating-systems/VERIFY_MODE_LOCK",
    originatingSystemPayload({ pricingMode: "DEALER_PRICED" })
  );
  assert.equal(changedWhileUnused.statusCode, 200);
  assert.equal(changedWhileUnused.body.pricingMode, "DEALER_PRICED");

  const attachedContext = await request("POST", "/api/v1/trade-contexts", {
  ...
  assert.match(rejectedModeChange.body.message, /used by Trade Context/);

  const systemsAfterRejection = await request("GET", "/api/v1/originating-systems");
  const persistedSystem = systemsAfterRejection.body.find(
    system => system.originatingSystemId === "VERIFY_MODE_LOCK"
  );
  assert.deepEqual(
    {
      ...
      tradeContextCount: persistedSystem.tradeContextCount
    },
    {
      name: "Allowed Metadata Update",
      pricingMode: "DEALER_PRICED",
  ...
  const contextId = encodeURIComponent(attachedContext.body.tradeContextId);
  const deletedContext = await request(
    "DELETE",
    `/api/v1/trade-contexts/${contextId}`
  );
...
test("Originating System editor disables Pricing Mode only for referenced rows", () => {
  ...
  assert.match(
    referencedMarkup,
    /aria-label="Pricing Mode: DEALER APPROVED\. Pricing Mode is locked while Trade Contexts are attached\./
  );
  assert.match(referencedMarkup, /data-tooltip="Pricing Mode is locked while Trade Contexts are attached\./);
