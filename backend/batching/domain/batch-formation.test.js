...
test("creates the expected mirrored pair for Trade IDs 5 and 16", () => {
  ...
  assert.equal(result.positionManagementMode, "MANUAL");
...
test("accepts any trade type when the trade has a positive Transfer Rate", () => {
  const result = formBatch({
    trades: [{
      ...commonTerms,
      tradeId: 28,
      tradeType: "FUTURE_TRADE",
      side: "SELL",
      baseCcyAmountMinor: 10000000n,
      quoteCcyAmountMinor: 11200000n,
      transferRate: 1.12
...
test("rejects any trade without a positive Transfer Rate", () => {
...
test("requires every source Trade to have a valid Position Management Mode", () => {
  assert.throws(
    () => formBatch({
      trades: [{
        ...commonTerms,
    ...
    error => error.code === "INVALID_POSITION_MANAGEMENT_MODE"
  );
});
...
test("rejects source Trades from different Position Management Modes", () => {
  assert.throws(
    ...
    error => error.code === "INCOMPATIBLE_BATCH_SELECTION"
      && /same current Position Management Mode/.test(error.message)
  );
});
