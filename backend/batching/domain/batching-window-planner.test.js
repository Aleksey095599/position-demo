...
test("opens a Batching Window with the first Trade and keeps it open until a trigger", () => {
  ...
  assert.equal(windows[0].status, BATCHING_WINDOW_STATUS.OPEN);
...
test("rejects Carry-in Position as a Batching Window trade", () => {
  assert.throws(
    ...
    error => {
      assert.equal(error.code, "INVALID_BATCHING_WINDOW_PLAN");
...
test("closes at the exact Maximum Batching Interval and opens a later Trade in a new window", () => {
  ...
  assert.equal(windows[0].status, BATCHING_WINDOW_STATUS.CLOSED);
  assert.equal(
  ...
  assert.equal(windows[1].status, BATCHING_WINDOW_STATUS.OPEN);
...
test("does not let a future Trade close a Batching Window", () => {
  assert.throws(
    ...
    error => error?.code === "INVALID_BATCHING_WINDOW_PLAN"
