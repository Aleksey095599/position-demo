...
const {
  ...
  planBatchingWindows
} = require("./batching-window-planner");
const {
...
function policyError(message) {
  ...
  error.code = "INVALID_AUTO_BATCHING_POLICY";
  return error;
}
...
function normalizedNow(value) {
  ...
  if (!Number.isFinite(date.getTime())) {
    throw policyError("Auto Batching requires a valid evaluation time.");
  }
...
function selectedBatchSources({
  ...
  // Резервируем место входящей сделке: один Carry-in не формирует Batch.
...
function arrivedTradesAt(trades, evaluationTime) {
  ...
  trades.forEach(trade => {
    ...
    if (!Number.isFinite(receivedAtMilliseconds)) {
      throw policyError(
        `Trade ${trade.tradeId} Received Timestamp must be a valid timestamp.`
      );
    }
...
function planAutoBatching({
  ...
  if (
    ...
    throw policyError(
      "Auto Batching currently supports SAME_TENOR_ONLY Tenor Compatibility."
    );
  }
  ...
  const eligibleTrades = selectEligibleTrades(trades, maximumTrades);
  ...
  for (const [ccyPairCode, groups] of pairGroups) {
    ...
    for (const group of groups) {
      ...
      const windows = planBatchingWindows({
        trades: windowTrades,
        maxSpreadPercent,
      ...
      for (const window of windows) {
        if (window.status === BATCHING_WINDOW_STATUS.OPEN) {
          openWindowCount += 1;
