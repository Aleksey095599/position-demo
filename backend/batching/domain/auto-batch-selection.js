...
function selectEligibleTrades(
  trades,
  maxTrades = DEFAULT_MAX_TRADES_PER_BATCH
) {
  if (!Array.isArray(trades)) {
    throw new TypeError("Auto Batching requires a Trade collection.");
  }

  if (!Number.isInteger(maxTrades) || maxTrades <= 0) {
    throw new RangeError("Maximum trades per Batch must be a positive integer.");
  }

  return trades
    .filter(isEligibleTrade)
    .sort(compareByAge);
}
...
function selectAutoBatchCandidatesByCurrencyPair(
  ...
  const eligibleTrades = selectEligibleTrades(trades, maxTrades);
