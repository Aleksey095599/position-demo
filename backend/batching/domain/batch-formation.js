...
const {
  ...
  "../../position-management/domain/position-management-policy"
);
...
function commonPositionManagementMode(trades) {
  ...
  if (trades.some(trade =>
    ...
    throw batchFormationError(
      "INCOMPATIBLE_BATCH_SELECTION",
      "All source Trades in one Batch must have the same current Position Management Mode."
    );
  }
...
function assertValidBatchSourceTrades(trades) {
...
function formBatch({
  ...
  const positionManagementMode = commonPositionManagementMode(normalizedTrades);
  ...
  normalizedTrades.forEach(trade => {
    const balanceContributions = tradeBalanceContributionsMinor(trade);
  ...
  const balanceTradeContributions = tradeBalanceContributionsMinor({
    ...balanceTrade,
