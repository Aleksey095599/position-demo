...
const AUTO_BATCHING_MAX_INTERVAL_SECONDS_MIN = 1;
const AUTO_BATCHING_MAX_INTERVAL_SECONDS_MAX = 3600;
const AUTO_BATCHING_MAX_INTERVAL_SECONDS_DEFAULT = 60;
const AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MIN = "0.0001";
const AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MAX = "100";
const AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_DEFAULT = "0.05";
const AUTO_BATCHING_TENOR_COMPATIBILITY_MODE = Object.freeze({
  SAME_TENOR_ONLY: "SAME_TENOR_ONLY"
});
const AUTO_BATCHING_TENOR_COMPATIBILITY_MODE_DEFAULT =
  AUTO_BATCHING_TENOR_COMPATIBILITY_MODE.SAME_TENOR_ONLY;
const AUTO_BATCHING_CCY_PAIR_CODES_DEFAULT = Object.freeze([
  "EUR_USD",
  "GBP_USD"
]);
const AUTO_BATCHING_CCY_PAIR_CODE_PATTERN = /^[A-Z]{3}_[A-Z]{3}$/;

function invalidSettings(message) {
  const error = new RangeError(message);
  error.code = "INVALID_AUTO_BATCHING_SETTINGS";
  return error;
}
  ...
  try {
    ...
    throw invalidSettings(
      "Default Transfer Rate Corridor must be a decimal percentage from "
        + `${AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MIN} to `
        + `${AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MAX}.`
    );
  }

  if (
    spreadPercent.lt(AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MIN)
    || spreadPercent.gt(AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MAX)
  ) {
    throw invalidSettings(
      "Default Transfer Rate Corridor must be a decimal percentage from "
        + `${AUTO_BATCHING_MAX_TRANSFER_RATE_SPREAD_PERCENT_MIN} to `
...
function normalizedEligibleCcyPairCodes(value) {
  ...
  if (
    codes.length === 0
    || codes.some(code => !AUTO_BATCHING_CCY_PAIR_CODE_PATTERN.test(code))
  ) {
    throw invalidSettings(
...
function normalizedTenorCompatibilityMode(value) {
  ...
  if (mode !== AUTO_BATCHING_TENOR_COMPATIBILITY_MODE.SAME_TENOR_ONLY) {
    throw invalidSettings(
...
function autoBatchingSettings(source) {
  const maxIntervalSeconds = Number(source?.maxIntervalSeconds);

  if (
    !Number.isInteger(maxIntervalSeconds)
    || maxIntervalSeconds < AUTO_BATCHING_MAX_INTERVAL_SECONDS_MIN
    || maxIntervalSeconds > AUTO_BATCHING_MAX_INTERVAL_SECONDS_MAX
  ) {
    throw invalidSettings(
      `Maximum Batching Interval must be a whole number of seconds from `
        + `${AUTO_BATCHING_MAX_INTERVAL_SECONDS_MIN} to `
...
module.exports = {
  AUTO_BATCHING_CCY_PAIR_CODE_PATTERN,
  ...
  AUTO_BATCHING_TENOR_COMPATIBILITY_MODE,
