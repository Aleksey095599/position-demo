...
function batchFormationTiming({
  reasonCode,
  windowOpenedAt,
  windowClosedAt
}) {
  if (reasonCode === BATCH_FORMATION_REASON_CODE.MANUAL_SELECTION) {
    if (windowOpenedAt !== null && windowOpenedAt !== undefined) {
      throw timingError("A manually formed Batch cannot have Window Opened At.");
    }

    if (windowClosedAt !== null && windowClosedAt !== undefined) {
      throw timingError("A manually formed Batch cannot have Window Closed At.");
    }
