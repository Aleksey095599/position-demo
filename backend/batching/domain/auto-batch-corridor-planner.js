...
} = require("./transfer-rate-corridor");

const AUTO_BATCH_CORRIDOR_TRIGGER_REASON = Object.freeze({
  TRANSFER_RATE_CORRIDOR_BREACHED: "TRANSFER_RATE_CORRIDOR_BREACHED"
});

function planningError(message) {
  const error = new RangeError(message);
  error.code = "INVALID_AUTO_BATCH_CORRIDOR_PLAN";
  return error;
}
...
function normalizedTrade(trade, index) {
  ...
  if (!Number.isSafeInteger(tradeId) || tradeId <= 0) {
    throw planningError(`Trade ${index + 1} must have a positive integer ID.`);
  }

  if (!Number.isFinite(receivedTimestamp)) {
    throw planningError(`Trade ${tradeId} must have a valid Received Timestamp.`);
  }

  if (!ccyPairCode) {
    throw planningError(`Trade ${tradeId} must have a Ccy Pair Code.`);
  }
...
function planAutoBatchByTransferRateCorridor({
  ...
  if (!Array.isArray(trades)) {
    throw new TypeError("Auto Batch corridor planning requires a Trade collection.");
  }
  ...
  normalizedTrades.forEach(trade => {
    if (tradeIds.has(trade.tradeId)) {
      throw planningError(`Trade ID ${trade.tradeId} is duplicated.`);
    }
  ...
  for (let index = 0; index < normalizedTrades.length; index += 1) {
    ...
    if (attemptedCorridor.isBreached) {
      return frozenPlan({
        shouldBatch: true,
        reason:
          AUTO_BATCH_CORRIDOR_TRIGGER_REASON.TRANSFER_RATE_CORRIDOR_BREACHED,
        ccyPairCode,
...
module.exports = {
  AUTO_BATCH_CORRIDOR_TRIGGER_REASON,
