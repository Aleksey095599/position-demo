...
const {
  formBatch
} = require("../domain/batch-formation");
const {
  batchFormationReason
} = require("../domain/batch-formation-reason");
const {
  batchFormationTiming
} = require("../domain/batch-formation-timing");
...
function normalizedCommand(command) {
  ...
  if (/^__(?:fx_)?manual_batch__:/.test(idempotencyKey)) {
    throw applicationError(
      "INVALID_BATCH_COMMAND",
  ...
  if (!Array.isArray(source.tradeIds) || source.tradeIds.length === 0) {
    throw applicationError(
      "INVALID_BATCH_COMMAND",
      "Select at least one eligible Trade."
    );
  }
  ...
  const formationReason = batchFormationReason({
    reasonCode: source.formationReasonCode,
    details: source.formationReasonDetails
  }, tradeIds.length);
  const formationTiming = batchFormationTiming({
...
class FormBatchUseCase {
  constructor({
    ...
    this.tradeExposureRepository = tradeExposureRepository;
    this.clock = clock;
  }
  ...
  #executeNormalizedWithinTransaction(normalized) {
    const existing = this.batchRepository.findFormedByIdempotencyKey(
    ...
    if (existing) {
      ...
      if (
        ...
        throw applicationError(
          ...
          "Idempotency Key was already used for a different Batch command."
        );
      }
    ...
    const sourceTrades = this.tradeExposureRepository.findBatchSources(
      normalized.tradeIds
    );
    ...
    if (!sameTradeIds(sourceTradeIds, normalized.tradeIds)) {
      ...
      if (missingTradeIds.length > 0) {
        throw applicationError(
          ...
          `Trade IDs unavailable for Batch formation: ${missingTradeIds.join(", ")}.`
        );
      }

      throw applicationError(
        "INVALID_BATCH_COMMAND",
        "Provided source Trades must match the command Trade IDs."
      );
    }

    const formation = formBatch({
      trades: sourceTrades,
      rateFractionDigits: sourceTrades[0]?.rateFractionDigits,
      now: this.clock
    });
    const result = this.batchRepository.saveFormed({
