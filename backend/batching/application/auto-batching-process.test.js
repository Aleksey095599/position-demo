...
const {
  AutoBatchingProcess
} = require("./auto-batching-process");

test("forms one independent Batch per currency pair after each configured interval", async () => {
  const scheduled = [];
  const commands = [];
  let batchId = 10;
  let idempotencySequence = 0;
  const process = new AutoBatchingProcess({
    selectCandidates: () => ({
      candidates: [
        {
          ccyPairCode: "EUR_USD",
...
test("excludes rolled-back Trades from the current Auto Batching run", async () => {
  const scheduled = [];
  const selections = [];
  const process = new AutoBatchingProcess({
    selectCandidates: criteria => {
  ...
  process.start();
  process.excludeTradesFromCurrentRun([11, 12]);
  await scheduled[0].callback();
