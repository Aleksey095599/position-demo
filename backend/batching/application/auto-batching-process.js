...
class AutoBatchingProcess {
  constructor({
    ...
    if (typeof selectCandidates !== "function") {
      throw new TypeError("Auto Batching Process requires a per-currency-pair selection query.");
    }

    if (typeof formBatch !== "function") {
      throw new TypeError("Auto Batching Process requires a form-batch use case.");
    }

    if (typeof getIntervalMs !== "function") {
      throw new TypeError("Auto Batching Process requires a batching-interval provider.");
    }

    if (typeof getLatestTradeId !== "function") {
      throw new TypeError("Auto Batching Process requires a latest Trade ID provider.");
    }

    if (typeof createIdempotencyKey !== "function") {
      throw new TypeError("Auto Batching Process requires an Idempotency Key factory.");
    }
  ...
  async tick() {
    ...
    try {
      ...
      if (!Array.isArray(candidates)) {
        throw new TypeError("Auto Batching selection must return a candidate collection.");
      }
      ...
      if (!Number.isInteger(openWindowCount) || openWindowCount < 0) {
        throw new RangeError(
          "Auto Batching open Batching Window count must be a non-negative integer."
        );
      }
  ...
  excludeTradesFromCurrentRun(tradeIds) {
    const tradeIdsToExclude = tradeIdCollection(
      tradeIds,
      "Trades excluded from the current Auto Batching run"
    );
    ...
    tradeIdsToExclude.forEach(tradeId => {
      this.excludedTradeIds.add(tradeId);
    });
