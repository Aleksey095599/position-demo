...
function migrateLegacyManualBatchFormations(database, {
  ...
  const formationsExist = tableExists(
    database,
    "manual_batch_formations"
  );
  const linksExist = tableExists(
    database,
    "manual_batch_formation_batches"
  );
  ...
  if (!formationsExist || !linksExist) {
    throw new Error("Incomplete legacy manual Batch formation schema.");
  }

  const migrationResult = runInImmediateTransaction(database, () => {
    if (database.prepare("PRAGMA foreign_key_check").all().length > 0) {
      throw new Error(
        "Legacy manual Batch schema contains foreign key violations."
      );
    }
    ...
    const linksByFormation = database.prepare(`
      SELECT
        ...
        batch_id AS batchId
      FROM manual_batch_formation_batches
      WHERE formation_id = ?
    ...
    for (const formation of formations) {
      ...
      for (const link of links) {
        ...
        if (
          ...
          || batch.idempotencyKey.replace(/^__fx_manual_batch__:/, "__manual_batch__:")
            !== `__manual_batch__:${formationId}:${batchOrdinal}`
        ) {
          throw invalidLegacyOperation(
            formationId,
            `does not match Batch ${batchId}.`
          );
        }
      ...
      if (conflict) {
        throw new Error(
          `Manual batching Idempotency Key ${formation.idempotencyKey} `
            + `is already used by Batch ${conflict.batchId}.`
        );
      }
      ...
      if (Number(update.changes) !== 1) {
        throw new Error(
          `Legacy Batch ${primaryBatchId} Idempotency Key was not migrated.`
        );
      }
    ...
    database.exec(`
      DROP TABLE manual_batch_formation_batches;
      DROP TABLE manual_batch_formations;
    `);

    if (database.prepare("PRAGMA foreign_key_check").all().length > 0) {
      throw new Error(
        "Manual Batch migration produced foreign key violations."
      );
    }
  ...
  if (migrationResult.retiredSplitFormationIds.length > 0) {
    logger.warn(
      "Retired legacy split manual Batch operations "
        + `${migrationResult.retiredSplitFormationIds.join(", ")}; `
        + "all child Batches were preserved and each public key remains reserved."
    );
  }
