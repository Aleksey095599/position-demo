...
const AUTO_MANAGEMENT_ADMISSION_STATE = Object.freeze({
  HELD: "HELD",
  RELEASED: "RELEASED"
});

const AUTO_MANAGEMENT_ELIGIBILITY_CHECK = Object.freeze({
  CCY_PAIR_ELIGIBILITY: "CCY_PAIR_ELIGIBILITY",
  TRANSFER_RATE_DEVIATION: "TRANSFER_RATE_DEVIATION"
});

const AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS = Object.freeze({
  PASS: "PASS",
  FAIL: "FAIL",
...
function ccyPairEligibilityCheck({
  ...
  if (!pairRule || pairRule.enabled === false
    ...
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.CCY_PAIR_ELIGIBILITY,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.FAIL,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.CCY_PAIR_NOT_ENABLED,
  ...
  if (!limit || limit.eq("0")) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.CCY_PAIR_ELIGIBILITY,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.FAIL,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.TRADE_AMOUNT_LIMIT_NOT_CONFIGURED,
      evidence: { ccyPairCode: normalizedPairCode }
    });
  }
  ...
  if (!amount) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.CCY_PAIR_ELIGIBILITY,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.UNAVAILABLE,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.TRADE_AMOUNT_UNAVAILABLE,
      evidence: {
  ...
  return frozenCheck({
    ...
    status: isWithinLimit
      ? AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.PASS
      : AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.FAIL,
    reasonCode: isWithinLimit
      ? AUTO_MANAGEMENT_ADMISSION_REASON.ELIGIBLE
      : AUTO_MANAGEMENT_ADMISSION_REASON.TRADE_AMOUNT_LIMIT_EXCEEDED,
    evidence: {
      ccyPairCode: normalizedPairCode,
...
function transferRateDeviationCheck({
  ...
  if (!threshold) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.TRANSFER_RATE_DEVIATION,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.UNAVAILABLE,
      reasonCode:
        AUTO_MANAGEMENT_ADMISSION_REASON.TRANSFER_RATE_DEVIATION_LIMIT_NOT_CONFIGURED,
      evidence: {}
    });
  }
  ...
  if (threshold.lt("0") || threshold.gt("100")) {
    ...
    error.code = "INVALID_AUTO_MANAGEMENT_ADMISSION_POLICY";
    throw error;
  }
  ...
  if (!(["BUY", "SELL"].includes(normalizedSide))) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.TRANSFER_RATE_DEVIATION,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.UNAVAILABLE,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.TRADE_SIDE_UNSUPPORTED,
  ...
  if (normalizedStreamStatus !== "RUNNING" || !applicableMarketRate) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.TRANSFER_RATE_DEVIATION,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.UNAVAILABLE,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.MARKET_PULSE_UNAVAILABLE,
      evidence: {
        side: normalizedSide,
  ...
  if (!normalizedTransferRate) {
    return frozenCheck({
      code: AUTO_MANAGEMENT_ELIGIBILITY_CHECK.TRANSFER_RATE_DEVIATION,
      status: AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.UNAVAILABLE,
      reasonCode: AUTO_MANAGEMENT_ADMISSION_REASON.TRANSFER_RATE_UNAVAILABLE,
      evidence: {
        side: normalizedSide,
  ...
  return frozenCheck({
    ...
    reasonCode: isWithinLimit
      ? AUTO_MANAGEMENT_ADMISSION_REASON.ELIGIBLE
      : AUTO_MANAGEMENT_ADMISSION_REASON.TRANSFER_RATE_DEVIATION_EXCEEDED,
    evidence: {
      side: normalizedSide,
      marketRateSide: normalizedSide === "BUY" ? "OFFER" : "BID",
...
function heldDecision(mode, reasonCode, releasable) {
  return Object.freeze({
    state: AUTO_MANAGEMENT_ADMISSION_STATE.HELD,
    admissionMode: mode,
    releasable,
...
function eligibilityDecision(source, admissionMode) {
  ...
  const failedReasonCodes = checks
    .filter(check => check.status !== AUTO_MANAGEMENT_ELIGIBILITY_CHECK_STATUS.PASS)
    .map(check => check.reasonCode);
  const eligible = failedReasonCodes.length === 0;

  return Object.freeze({
    state: eligible
      ? AUTO_MANAGEMENT_ADMISSION_STATE.RELEASED
      : AUTO_MANAGEMENT_ADMISSION_STATE.HELD,
    admissionMode,
    releasable: !eligible,
    reasonCodes: Object.freeze(
      eligible ? [AUTO_MANAGEMENT_ADMISSION_REASON.ELIGIBLE] : failedReasonCodes
    ),
    checks
  });
}
