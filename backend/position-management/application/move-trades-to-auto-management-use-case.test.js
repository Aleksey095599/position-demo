...
} = require("./move-trades-to-auto-management-use-case");
...
function useCaseWith({
  ...
  return new MoveTradesToAutoManagementUseCase({
    transactionRunner: transactionRunner || {
      run: operation => operation()
    },
    tradePositionManagementRepository: repository || {
      findByIdentities: identities => identities
        .map(identity => stateByIdentity.get(identityKey(identity)))
        .filter(Boolean),
      saveTransition: () => {}
    },
    admissionPolicy: admissionPolicy || { evaluateRelease: () => ({ state: "RELEASED" }), recordRelease: () => {} },
    clock
  });
}
...
test("distinguishes a missing composite Trade identity", () => {
  ...
  assert.throws(
    ...
    error => error?.code === "POSITION_TRADE_NOT_FOUND"
...
test("enforces a unique composite identity and the 200 Trade limit", () => {
  assert.equal(MAX_TRADES_PER_POSITION_MODE_TRANSITION, 200);
  const useCase = useCaseWith();
...
test("rejects Trade Types outside CLIENT_DEAL and HEDGE_DEAL", () => {
  ...
  for (const tradeType of ["BATCH_BALANCE_TRADE", "BATCH_POSITION_OUT"]) {
    assert.throws(
      () => useCase.execute({
        trades: [{ tradeId: 1, tradeType }]
      }),
      error => error?.code ===
        "INVALID_POSITION_MODE_TRANSITION_COMMAND"
    );
