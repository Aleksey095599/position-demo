...
const loaded={candleCount:599,completedAt:"2026-09-21T12:04:00Z",firstCandleAt:"2026-09-15T06:00:00Z",lastCandleAt:"2026-09-15T15:59:00Z"};
...
test("source failures show a readable explanation while preserving the technical error",()=>{
 ...
 const result=details({candleCount:0,lastError},"ONE_MINUTE","ERROR");
 assert.match(result.message,/Could not retrieve data from MOEX ISS/);assert.doesNotMatch(result.message,/script|MOEX_ISS_/);
 assert.equal(result.technicalError,lastError);assert.equal(result.rows.length,0);
});
