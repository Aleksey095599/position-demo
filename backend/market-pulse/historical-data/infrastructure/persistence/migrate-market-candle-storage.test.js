...
function setup(t) {
  ...
  db.exec(`CREATE TABLE market_source_candles AS SELECT * FROM moex_iss_minute_candles;
    ...
    CREATE TABLE market_candle_load_ranges AS SELECT * FROM moex_iss_daily_candle_load_ranges;`);
  return db;
}
...
test("copies minutes and days separately, retains legacy rows, and is idempotent",t=>{
  ...
  for(const name of ['moex_iss_minute_candles','moex_iss_daily_candles','moex_iss_daily_candle_load_ranges']) assert.equal(db.prepare(`SELECT COUNT(*) n FROM ${name}`).get().n,1);
