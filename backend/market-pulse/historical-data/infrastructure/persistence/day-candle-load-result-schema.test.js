...
test("day load result contains only the instrument, date and load outcome",t=>{
  ...
  assert.deepEqual(columns.map(column=>column.name),["instrument_id","load_date","completed_at","last_error"]);
...
test("day results allow failure without a candle and reject invalid dates or duplicate keys",t=>{
  const db=setup(t);
  const insert=db.prepare("INSERT INTO moex_iss_day_candle_load_result (instrument_id,load_date,last_error) VALUES ('CNYRUB_TOM',?,'Unavailable')");
