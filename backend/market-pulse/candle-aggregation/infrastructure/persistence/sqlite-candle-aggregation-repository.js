...
const { aggregationTimeframe } = require("../../domain/aggregation-timeframe");
...
class SqliteCandleAggregationRepository {
  ...
  replaceDay({ instrumentId, timeframe, date, from, till, candles, calculatedAt, sourceLoadedAt }) {
    ...
    try {
      ...
      this.database.prepare(`INSERT INTO moex_iss_candle_aggregation_result
        ...
        last_error=NULL`)
        .run(instrumentId, timeframe, date, calculatedAt, sourceLoadedAt);
      this.database.exec("COMMIT");
    } catch (error) { this.database.exec("ROLLBACK"); throw error; }
  }

  recordFailure({ instrumentId, timeframe, date, error }) {
    this.database.prepare(`INSERT INTO moex_iss_candle_aggregation_result
      (instrument_id,timeframe,calculation_date,last_error) VALUES (?,?,?,?)
      ON CONFLICT (instrument_id,timeframe,calculation_date) DO UPDATE SET
      last_error=excluded.last_error`)
      .run(instrumentId, timeframe, date, String(error).slice(0, 1000));
  }
  ...
  findDaySummaries({ instrumentId, timeframe, fromDate, throughDate }) {
    ...
    const days = new Map(rows.map(row => [row.date, { ...row, candleCount: 0, partialCount: 0, insufficientCount: 0 }]));
    ...
    for (const row of this.database.prepare(`SELECT date(begin_at,'+3 hours') date,COUNT(*) candleCount,
      SUM(CASE WHEN component_count>=? AND component_count<? THEN 1 ELSE 0 END) partialCount,
      SUM(CASE WHEN component_count<? THEN 1 ELSE 0 END) insufficientCount FROM moex_iss_aggregated_candles
      WHERE instrument_id=? AND timeframe=? AND begin_at>=? AND begin_at<? GROUP BY date(begin_at,'+3 hours')`)
      .all(minimumMinutes, expectedMinutes ?? minimumMinutes, minimumMinutes, instrumentId, timeframe, from, till)) days.set(row.date, { ...days.get(row.date), ...row });
    return [...days.values()];
  }
